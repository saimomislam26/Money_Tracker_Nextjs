...
export const setMonthlyIncomeFirstTime = async (info: {
  year: Number,
  month: Number,
  income: Number | string
}) => {
...
export const verifyToken = async (token:string) => {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/user/verify/${token}`);
...
export const passwordResetRequest = async (email:string) => {
  try {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/user/reset-password-request`,{email});
...
export const changeForgotPassword = async(password:string, token:string)=>{
  try {
